<template>
  <div class="wrapper" v-if="!isItemsLoading">
    <span class="title">Formules de physique moléculaire en français</span>
    <div class="list">
      <div class="legend">
        <span style="flex: 1">Français</span>
        <span style="flex: 1">Russe</span>
        <span style="flex: 2">Formule</span>
        <span style="flex: 3">Commentaire</span>
      </div>
  ...
  methods: {
    async fetch() {
      ...
      this.items = [...JSON.parse(JSON.stringify(
          await formulasAPI.getFormulasList('frn', this.page, 'molecular'))).rows]
