<template>
  <div class="wrapper" v-if="!isItemsLoading">
    <span class="title">Electrodynamics formulas in English</span>
    <div class="list">
      <div class="legend">
        <span style="flex: 1">English</span>
        <span style="flex: 1">Russian</span>
        <span style="flex: 2">Formula</span>
        <span style="flex: 3">Comment</span>
      </div>
  ...
  methods: {
    async fetch() {
      ...
      this.items = [...JSON.parse(JSON.stringify(
          await formulasAPI.getFormulasList('eng', this.page, 'electrodynamics'))).rows]
