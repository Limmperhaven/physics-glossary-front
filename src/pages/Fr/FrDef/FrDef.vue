<template>
  <div class="form-outline my-input-search">
    <input type="search" id="form1" class="form-control" placeholder="Search definition..." aria-label="Search" />
  </div>
  <div class="wrapper" v-if="!isItemsLoading">
    ...
    <div class="list">
      <div class="legend">
        ...
        <span style="flex: 1">Russe</span>
        <span style="flex: 2">Explication en français</span>
        <span style="flex: 2">Explication en russe</span>
      </div>
  ...
  methods: {
    async fetch() {
      ...
      this.items = [...JSON.parse(JSON.stringify(
          await definitionsAPI.getDefinitionsList('frn', this.page, 'dynamics'))).rows]
      console.log('+')
